...
attempt_clear w = let  bls = game_blocks w
                  ...
                  in
                      World 
                      (active_tetromino w) 
                      ([if (snd . block_location) b > lr 
                          then  apply_gravity b w (length fs)
                          else b
                              | b <- filter (\b -> not 
                                             $ (snd . block_location) b 
                                                   `elem` fs) bls])
                      (upcoming_tetrominos w)
                      (game_score w + if ls >= 4 
                                        then 2 * ls 
                                        else ls) 
                      (lock_timer w) (world_step w)
...
next_frame _ w
    | any (((==) 0) . snd) bls || any (==True) ((==) <$> bls <*> gbls) = 
        if lock_timer w <= 0 
          ...
          else World (active_tetromino w) gbs (upcoming_tetrominos w) (game_score w) 
                   (lock_timer w - 1) (world_step w)
    | world_step w == 59 = World (attempt_translate t ShiftDown w) gbs
                           (upcoming_tetrominos w) (game_score w) lOCK_TIME 0
    | otherwise = World t gbs (upcoming_tetrominos w) (game_score w) 15 
                  (world_step w + 1)
  where
